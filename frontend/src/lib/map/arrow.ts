 ...
 * arrow (`_draw_motion_arrow` in `src/dmi_nowcast_core/render.py`), and it
 ...
 *   - it is **one hour long** — `speedKmh × 1 h` of ground — so the arrowhead
 *     sits on top of whatever will arrive in ~60 minutes, if the motion holds;
 *   - the shaft is ticked at every radar timestep (the manifest's
 *     `timestep_min`: 10 min → ticks at 10, 20, 30, 40, 50), each tick marking
 *     where the rain that arrives in that many minutes is *now*. The tip is
 *     the 60 min mark and carries the head instead of a tick.
 *
...
/**
 * How far the arrow reaches, in minutes of travel. Sixty, because "where is
 * the rain that hits me in an hour" is the question the arrow answers.
 */
...
export type ArrowRole = 'shaft' | 'head' | 'tick';
...
export type ArrowProperties = {
	role: ArrowRole;
	/** Minutes of travel this feature marks. Shaft and head carry the horizon. */
	minute: number;
};
...
export interface MotionArrowInput {
	...
	/** Speed over the ground, km/h. One hour of it is the arrow's length. */
	speedKmh: number;
...
/**
 * Build the arrow for one sampled motion: shaft, arrowhead, and one tick per
 * radar timestep short of the tip.
 *
...
export function motionArrow(input: MotionArrowInput): ArrowCollection {
	const { lat, lon, bearingFromDeg, speedKmh, timestepMin } = input;
	...
	const lengthKm = speedKmh * (ARROW_HORIZON_MIN / 60);
	...
	if (Number.isFinite(timestepMin) && timestepMin >= 1) {
		for (let i = 1; i * timestepMin < ARROW_HORIZON_MIN; i++) {
			...
			const [tLon, tLat] = destinationPoint(
				lat,
				lon,
				bearingFromDeg,
				lengthKm * (minute / ARROW_HORIZON_MIN)
			);
			features.push({
				type: 'Feature',
				properties: { role: 'tick', minute },
				geometry: {
					type: 'LineString',
					coordinates: [
						destinationPoint(tLat, tLon, bearingFromDeg - 90, tickHalfKm),
						destinationPoint(tLat, tLon, bearingFromDeg + 90, tickHalfKm)
					]
				}
			});

...
export const da = {
	...
	quality: {
		...
		stations: {
			...
			legendTitle: (national: string) =>
				`Andel af regnen vi varslede, målt mod landsgennemsnittet på ${national}`,
			legendTitleAbsolute: 'Andel af regnen vi varslede',
			legendNote:
				'Farverne sammenligner hver station med landsgennemsnittet: en prik under det ligger under gennemsnittet, ikke nødvendigvis en station, hvor varslerne svigter.',
			legendPoor: (edge: string) => `under ${edge}`,
			legendFair: (from: string, to: string) => `${from}–${to}`,
			legendGood: (from: string, to: string) => `${from}–${to}`,
			legendBest: (edge: string) => `over ${edge}`,

...
from ..decorators import check_input_frames

from ..feature.shitomasi import detection as _shitomasi_detection
from ..tracking.lucaskanade import track_features
from ..utils.cleansing import decluster, detect_outliers
from ..utils.images import morph_opening
from ..utils.interpolate import idwinterp2d as _idwinterp2d

# VENDORING CHANGE (see NOTICE, item 2). Upstream resolves ``fd_method``
# and ``interp_method`` through ``pysteps.feature.get_method`` and
# ``pysteps.utils.get_method``; those dispatchers eagerly import every
# detector and utility in the package, including the ones backed by
# Cython extensions. Only the two default methods are vendored, so the
# dispatch is a dict and an unknown name says so by name.
_FEATURE_METHODS = {"shitomasi": _shitomasi_detection}
_INTERP_METHODS = {"idwinterp2d": _idwinterp2d}


def _get_feature_method(name):
    try:
        return _FEATURE_METHODS[name]
    except KeyError:
        raise ValueError(
            f"unknown feature detection method {name!r}; vendored: "
            f"{', '.join(sorted(_FEATURE_METHODS))}"
        ) from None


def _get_interp_method(name):
    try:
        return _INTERP_METHODS[name]
    except KeyError:
        raise ValueError(
            f"unknown interpolation method {name!r}; vendored: "
            f"{', '.join(sorted(_INTERP_METHODS))}"
        ) from None
...
def dense_lucaskanade(
    ...
    feature_detection_method = _get_feature_method(fd_method)
    interpolation_method = _get_interp_method(interp_method)
